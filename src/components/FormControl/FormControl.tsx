...
export type Component = CInput | CTextarea | CSelect;
export const components = ["input", "textarea", "select"] as const;
export type componentType = typeof components[number];
export interface Props
  extends sharedInputProps,
    Omit<HTMLPropsNoClassName, "size"> {
  handleBlur: FocusEventHandler;
  handleChange: FocusEventHandler;
  values: FormikValues;
...
export type Ref = Component;

const getEyeIcon = (visible: boolean | null) => {
...
const Input = forwardRef<Ref, Props>(
  (
    ...
    const Component = useMemo<ElementType<Component>>(
      () =>
      ...
      <>
        <div className={`position-relative`}>
          <Component
            ...
            {...(componentType === "select" ? { custom: "true" } : {})}
            // custom="true"
            style={style}
            {...props}
          >
